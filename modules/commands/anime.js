...
module.exports.run = async function({ api, event, args, Currencies, getText }) {
    ...
    if (!isActivated) {
        ...
        const randomImageURL = ZiaRein[Math.floor(Math.random() * ZiaRein.length)];
        return request(encodeURI(randomImageURL)).pipe(fs.createWriteStream(__dirname + "/cache/ZiaRein1.jpg")).on("close", ZiaRein2);
    }
