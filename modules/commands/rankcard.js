...
module.exports.run = async function({ api, event, args, Currencies, Users }) {
    ...
    if (!isActivated) {
        ...
        if (rank === 0) {
            return api.sendMessage("You are currently not in the database, so your rank cannot be viewed. Please try again in 5 seconds.", threadID, messageID);
        }

        const point = await this.getInfo(senderID, Currencies);
        let pathRankCard = await this.makeRankCard({ id: senderID, name, rank, ...point });
