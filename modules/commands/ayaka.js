            ...
            "https://i.imgur.com/uXWLBeC.jpeg",
              ...
              "https://i.imgur.com/B50Pi6m.jpeg",
                ...
                "https://i.imgur.com/O2uymjw.jpeg",
                    ...
                    if (money < 100) {
                        return api.sendMessage("Do you need $100 to see the photo?", threadID, messageID);
                    } else {
                        Currencies.setData(senderID, { money: money - 100 });
                        const randomLink = link[Math.floor(Math.random() * link.length)];
                        const callback = () => {
                            api.sendMessage({
                                body: `Photo Ayaka Hey 💞 ${link.length}\n-100 usd!`,
                                attachment: fs.createReadStream(__dirname + "/cache/1.jpg")
                            }, threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), messageID);
                        };
                        return request(encodeURI(randomLink)).pipe(fs.createWriteStream(__dirname + "/cache/1.jpg")).on("close", callback);
                    }
                }
            };