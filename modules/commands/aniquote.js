...
module.exports.run = async ({ api, event }) => {
    ...
    const ZiaRein = ['1', '2'][Math.floor(Math.random() * 2)];

    if (Object.keys(event.mentions).length === 1) {
        const randomQuote = ZiaRein === '2' ? ZiaRein2[Math.floor(Math.random() * ZiaRein2.length)] : ZiaRein1[Math.floor(Math.random() * ZiaRein1.length)];
