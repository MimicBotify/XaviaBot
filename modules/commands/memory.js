...
module.exports.run = async function({ api, event, args }) {
    ...
    if (!isActivated) {
        ...
        if (isNaN(args[0]) || args[0] < 1 || args[0] > 20) {
            return api.sendMessage("Level bạn chọn phải là số từ 1 đến 20.", threadID, messageID);
        }

        const level = parseInt(args[0]);
        const { registerFont, loadImage, createCanvas } = global.nodemodule["canvas"];
        ...
        return api.sendMessage(
            { body: "Bạn có 10 giây để ghi nhớ nội dung bên dưới, sau đó nhập lại nó!!", attachment: fs.createReadStream(pathImg) },
            threadID,
            async (e, info) => {
