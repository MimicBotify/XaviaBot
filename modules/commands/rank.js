...
const { downloadFile } = require('global/utils');
...
module.exports.run = async function({ api, event, args, Currencies, getText, Users }) {
    ...
    if (!isActivated) {
        ...
        const listUserID = participantIDs;
        const exp = [];
        ...
        const rank = exp.findIndex(info => parseInt(info.uid) === parseInt(senderID)) + 1;
        const infoUser = exp[rank - 1];

        if (rank === 0) {
            return api.sendMessage("Your name is currently not available in the database. Maybe you're a newbie. Please increase your keyboard level by chatting in this group.\n\nCreated by: ZiaReinn", threadID, messageID);
        }

        const point = await this.getInfo(senderID, Currencies);
        const pathRankCard = await this.makeRankCard({ id: senderID, name, rank, ...point });

        return api.sendMessage({
            body: `Name: ${name}\nTop: ${rank}\nNumber of messages: ${infoUser.exp}`,
