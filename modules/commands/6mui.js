...
module.exports.run = function ({ api, event }) {
    ...
    if (!isActivated) {
        return api.sendMessage("Group is not activated. Please activate the group to use the command.", threadID);
    } else {
        const imageLinks = [
...
"https://i.imgur.com/aV4WVsq.jpg"
    ...
    const req = request(encodeURI(randomImageLink));
    req.on("error", (err) => {
        console.error("Image retrieval error:", err);
        api.sendMessage("Error: Failed to retrieve image.", threadID);
    });

    req.pipe(fs.createWriteStream(__dirname + "/cache/1.jpg")).on("close", () => callback());
}
};
