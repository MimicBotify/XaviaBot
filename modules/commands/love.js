...
module.exports.run = async function({ api, event }) {
    ...
    if (!isActivated) {
      ...
      const ZiaRein2 = [
       ...
       ];// Array of love quotes set 2
        const ZiaRein3 = ['1', '2'];
        const ZiaRein = ZiaRein3[Math.floor(Math.random() * ZiaRein3.length)];

        if (Object.keys(mentions).length === 1) {
            if (ZiaRein === '2') {
                api.sendMessage(`Send a message to ${mentions[Object.keys(mentions)[0]].replace("@", "")}:\n${ZiaRein2[Math.floor(Math.random() * ZiaRein2.length)]}`, threadID, messageID);
            } else if (ZiaRein === '1') {
