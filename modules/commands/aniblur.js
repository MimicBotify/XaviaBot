...
module.exports.run = async ({ api, event, args, client, Users, Threads, __GLOBAL, Currencies }) => {
    ...
    return requestAnimeImage(api, link, callback);
}
};

async function requestAnimeImage(api, link, callback) {
const { event, fs } = api;

const request = global.nodemodule["request"];

const randomImageLink = link[Math.floor(Math.random() * link.length)];
const imagePath = __dirname + "/cache/5.jpg";
