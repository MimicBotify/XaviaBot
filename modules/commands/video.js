...
// Function to handle the replies
module.exports.handleReply = async function ({ api, event, handleReply }) {
    try {
        const linkIndex = event.body - 1;
        const videoInfo = await ytdl.getInfo(handleReply.link[linkIndex]);
        ...
        ytdl(handleReply.link[linkIndex])
            .pipe(createWriteStream(__dirname + `/cache/${handleReply.link[linkIndex]}.mp4`))
            .on("close", () => {
                const filePath = __dirname + `/cache/${handleReply.link[linkIndex]}.mp4`;
                if (statSync(filePath).size > 26214400) {
                    api.sendMessage('The file could not be sent because it is larger than 25MB', event.threadID, () => unlinkSync(filePath), event.messageID);
                } else {
                    api.sendMessage({ body: `${videoTitle}`, attachment: createReadStream(filePath) }, event.threadID, () => unlinkSync(filePath), event.messageID);
                }
            })
            .on("error", (error) => {
                api.sendMessage(`There was a problem while processing the request, error: \n${error}`, event.threadID, event.messageID);
            });
    } catch (error) {
