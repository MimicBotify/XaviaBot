...
module.exports.run = async function ({ api, event, args }) {    
    ...
    if (!isActivated) {
        ...
        if (!mentions[0]) return api.sendMessage(`Please tag 1 person\n\nHow to use?\n${global.config.PREFIX}marry <@tag>\n\nExample:\n${global.config.PREFIX}marry @name\n\nCreated by: CaNdY (LaFhanGa chokra)`, threadID, messageID);
        else {
            const one = senderID, two = mentions[0];
            return makeImage({ one, two }).then(path => api.sendMessage({ body: "Wish you two happiness. Have a nice honeymoon!", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
        }
    }
